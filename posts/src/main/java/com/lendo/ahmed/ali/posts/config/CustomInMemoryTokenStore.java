...
public class CustomInMemoryTokenStore implements TokenStore {
    ...
    private static final int DEFAULT_FLUSH_INTERVAL = 20000;
    ...
    public void storeAccessToken(OAuth2AccessToken token, OAuth2Authentication authentication) {
        if (this.accessTokenStore.size() >= this.flushInterval) {
            flush();
        }
        ...
        if (token.getRefreshToken() != null && token.getRefreshToken().getValue() != null) {
            ...
            this.refreshTokenStore.put(token.getRefreshToken().getValue(), token.getRefreshToken());
        }
    }
    ...
    private void flush() {
        ...
        while (expiry != null) {
            OAuth2AccessToken token = accessTokenStore.get(expiry.getValue());
            if(token != null) {
